...
def test_existing_projections_excludes_rows_computed_after_the_cutoff(
    db, player, schedule
):
    """Seeds both a pre- and post-cutoff row so this can't pass by luck.

    A row regenerated by a later `pigskin projections refresh` run is stamped
    with a computed_at long after the cutoff kickoff even though its
    player_id/year/week still match the backtest scope -- that's the leak
    this filter closes.
    """
    db.query(DBNFLGame).filter_by(year=2026, week=2).update(
        {"kickoff_at": datetime(2026, 9, 14, 17, 0)}
    ...
    db.add(
        DBPlayerProjection(
            ...
            computed_at=datetime(2026, 9, 1, 12, 0),  # before kickoff
        )
    )
    db.add(
        DBPlayerProjection(
            ...
            computed_at=datetime(2026, 9, 20, 12, 0),  # after kickoff
        )
    )
    db.commit()
    ...
    assert "espn" not in ev["existing_projections"]
    assert "cutoff" in ev["existing_projections_filtered_reason"]


def test_existing_projections_unfiltered_and_reason_none_without_a_cutoff(
    db, player, schedule
):
    db.query(DBNFLGame).filter_by(year=2026, week=2).update(
        {"kickoff_at": datetime(2026, 9, 14, 17, 0)}
    ...
    db.add(
        DBPlayerProjection(
            ...
            source="model",
            projected_points=15.0,
            computed_at=datetime(2026, 9, 1, 12, 0),
        )
    )
    db.add(
        DBPlayerProjection(
            ...
            source="espn",
            projected_points=16.0,
            computed_at=datetime(2026, 9, 20, 12, 0),
        )
    )
    db.commit()
    ...
    ev = build_evidence(db, player.id, 2026, week=2)
    assert "model" in ev["existing_projections"]
    assert "espn" in ev["existing_projections"]
    assert ev["existing_projections_filtered_reason"] is None


def test_existing_projections_unfiltered_when_cutoff_date_is_unknown(db, player):
    """No schedule row for the cutoff week means no date to filter against --
    consistent with how the news block handles the same unresolvable case.
    """
    db.add(
        DBPlayerProjection(
            player_id=player.id,
